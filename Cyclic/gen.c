...
void commit(void) {
	...
	// Allocate a new primary register
	// and load a new value into it.
	...
	case auto_byte:		cgldlb(Q_val); break;
	case auto_word:		cgldlw(Q_val); break;
	case static_byte:	cgldsb(Q_val); break;
	case static_word:	cgldsw(Q_val); break;
	case globl_byte:	cgldgb(gsym(Q_name)); break;
...
static void genincptr(struct lvalue *lv, int inc, int pre) {
	...
	// No symbol
	if (!y) {
		// Swap rvalue and register around if needed
		if (!pre) cgswap();
		...
		// Discard the pointer & get back to rvalue if needed
		if (!pre) cgunldinc();
	}
...
void geninc(struct lvalue *lv, int inc, int pre) {
	...
	// Generate the rvalue now if it's postincrement
	if (!pre) {
		if (!y) cgldinc();
		genrval(lv);
		commit();
...
// Put the switch expression into
// the most suitable register.
...
void genstore(struct lvalue *lv) {
	...
	if (!lv->sym) {
		...
		// Bubble primary register down to secondary, so
		// we can do xx.a= xx.b= xx.c= value;
		cgregtosec();
	}

...
static void do_stmt(void) {
	...
	freeregs(1);			// Primary register is now empty
...
static void switch_stmt(void) {
	...
	genswtchval();			// Save its value somewhere suitable
	freeregs(0);			// Primary register is now empty, but
...
static void stmt(void) {
	...
	default:	expr(&lv, 0, 0); semi(); commit(); break;
	}
	freeregs(1);	// Primary register is now empty
}
